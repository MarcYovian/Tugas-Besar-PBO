...
public class Edit_Product2 extends javax.swing.JPanel {
    
    private editProduct data;
    private User dataUser;
    ResultSet rs; 

    public Edit_Product2() {
        initComponents();
...
//        tpDeskripsi.setOpaque(false);
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    if(namaProduk.equals("") || Kategori.equals("") || jumlah < 0 || deskripsi.equals("")){
                        ...
                        Main main = (Main) SwingUtilities.getWindowAncestor(Edit_Product2.this);
                        main.setForm(edit);
                    }
                } catch (Exception ex) {
        ...
        btnSubmit.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    String namaProduk = txtName.getText();
                    String kategori = txtKategori.getText();
                    int jumlahAkhir = Integer.parseInt(txtJumlah.getText());
                    String deskripsi = tpDeskripsi.getText();
                        
                    DatabaseConnection db = new DatabaseConnection();
                    
                    int total = count(idRak) - jumlah + jumlahAkhir;
                    rs = db.getData("SELECT id_Rak, Kapasitas FROM rak_gudang WHERE id_Rak = '" + idRak + "' AND Kapasitas <= '" + total + "'");
                    ...
                    if (rs.next()) {
                        ...
                        msc.setData(new Model_Message("Konfirmasi", "Apakah anda yakin untuk mengubah data barang ? "));
                        msc.eventSUBMIT(new ActionListener(){
                            @Override
                            public void actionPerformed(ActionEvent e) {
                                GlassPanePopup.closePopupLast();
                                String update = "UPDATE barang SET id_Rak = ? , Kategori_Barang = ?, deskripsi = ? WHERE id_Barang = ? ";
                                db.query("Berhasi;", "Data Berhasil Diupdate", update, idRak, kategori, deskripsi, idBarang);
                                ...
                                db.query("Insert Berhasil", "Berhasil Menambahkan Pencatatan Gudang Baru", insertNote, idRak, username, idBarang, idPelanggan, date, Status);
    ...
    public int count(int id_Rak){
        int total = 0;
        DatabaseConnection db = new DatabaseConnection();
        rs = db.getData("SELECT * FROM barang WHERE id_Rak = '" + id_Rak + "'");
        try {
            while (rs.next()) {
